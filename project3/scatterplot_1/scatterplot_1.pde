...
void SelectFile(File selectedFile ){                                                 //funtion to load file
 ...
 for(int i=1;i<table.getRowCount();i++)
  {
   float adjcol2 =map(table.getFloat(i,1),minvalY, maxvalY,0, vizHeight-20);                     //map function is used to adjust values
   float yPos = height -space-adjcol2;
   float adjcol1= map(table.getFloat(i,0),minvalX, maxvalX,0, vizWidth-20);
...
float findMaxValcol1(){                                                   //finding maximum value of X-axis elements
  maxvalX=0.0;
  for(int i=1;i<table.getRowCount();i++)
    if(table.getFloat(i,0)>maxvalX)
      maxvalX=table.getFloat(i,0);
...
float findMaxValcol2(){                                                   //finding maximum value of Y-axis elements
  maxvalY=0.0;
  for(int i=1;i<table.getRowCount();i++)
    if(table.getFloat(i,1)>maxvalY)
      maxvalY=table.getFloat(i,1);
...
float findMinValcol1(){                                                   //finding minimum value of X-axis elements
  minvalX=table.getFloat(1,0);
  for(int i=1;i<table.getRowCount();i++)
    if(table.getFloat(i,0)<minvalX)
      minvalX=table.getFloat(i,0);
...
float findMinValcol2(){                                                   //finding minimum value of Y-axis elements
  minvalY=table.getFloat(1,1);
  for(int i=1;i<table.getRowCount();i++)
    if(table.getFloat(i,1)< minvalY)
      minvalY=table.getFloat(i,1);
